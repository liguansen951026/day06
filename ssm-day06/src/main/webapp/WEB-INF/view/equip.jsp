...
<body class="gray-bg">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveEquip.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							...
							<tr>
								<td>设备类型：</td>
								<td colspan="2">
									<select class="form-control" name="modelid" id = "roleid" aria-required="true" required>
										<option value="">---请选择---</option>
										<c:forEach items="${mList }" var="r">
										 	<option value="${r.id }">${r.modelname }</option>
										</c:forEach>
				                	</select>
								</td>
								<td>设备状态：</td>
	...
	function initEditUser(getSelection){
		...
		$('#roleid').val(getSelection.modelid);
